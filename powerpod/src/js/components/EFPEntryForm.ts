...
class EFPEntryForm extends LitElement {
  ...
  private buildStepHierarchy(currentStep: any): Array<{label: string, level: number}> {
  ...
  private findStepByPattern(pattern: RegExp): any {
            ...
            <li class="breadcrumb-item ${crumb.isActive ? 'active' : ''}">
              ${crumb.isActive ?
                html`<span
                  ...
                  title="${currentStep?.label || crumb.label}"
                  aria-current="page"
                >${crumb.label}</span>` :
                html`<button
